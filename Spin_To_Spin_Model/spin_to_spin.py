...
spin_state_total=np.loadtxt('spin_state_0.001000_different_init.txt',dtype=int)
...
J = 1
kB = 1
Ts = np.linspace( 0.001, .004, 30 ) * J / kB
n_record=1000
n_total = 200000
...
for num in range( len( Ts ) ):  # these are the temperatures
    count = np.zeros((Dist_len*L*2, 4))  # record the count
    ...
    MI = np.zeros((Dist_len))  # mutual information
    H_marginal = np.zeros((Dist_len))
    H_joint = np.zeros((Dist_len))

    #count the two spin states among monte carlo process
    for step in range(n_record):
        s = spin_state_total[(num)*n_record+step,:]

        #count spin states under different combinations and also different spin-to-spin distance
        for ave_num in range(ave):
            for distance_index, r2 in enumerate(Dist):
                loop = int(100 * distance_index)
                r = ave_num

                if (s[r] == 1 and s[(r+r2) % L] == 1):
                    count[ave_num+loop, 0] += 1
                elif (s[r] == 1 and s[(r+r2) % L] == -1):
    ...
    for ave_num in range(ave):
        for distance_index, r2 in enumerate(Dist):
            ...
            probability[3] = (count[ave_num+loop, 1] + count[ave_num+loop, 3]) / n_record if (count[ave_num+loop, 1] + count[
                ave_num+loop, 3]) != 0 else 1
            MI[distance_index] += -sum(probability * np.log2(probability)) + sum(probability_joint * np.log2(probability_joint))
            H_marginal[distance_index] += -sum(probability * np.log2(probability))
            H_joint[distance_index] += -sum(probability_joint * np.log2(probability_joint))
...
for distance in range(Dist_len):
